export default function getJQueryWrappers($) {
	let CE = getCachedJQueryElements();
	let VS = getValueSupplier(CE);
	let VC = getValueConsumer(CE);
	let VR = getValueResetter(CE);
	let VTS = getValueToString(CE);

	return { CE, VS, VC, VR, VTS };
}

function getCachedJQueryElements() {
	return {
		college: $('#colesel'),
		department: {},
...
function getValueSupplier(CE) {
	...
	return {
		...
		credits: basicValueSupplier.checkbox('credits', (e) => e),
